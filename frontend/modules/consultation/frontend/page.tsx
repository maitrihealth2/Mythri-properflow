        ...
        <div className="flex-1 overflow-y-auto pt-4 pb-48 md:pb-36 flex flex-col gap-6 hide-scrollbar pr-2" onClick={() => {setMenuOpen(false); setLangMenuOpen(false);}}>
          {messages.map((m, i) => (
            <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end self-end max-w-[90%] md:max-w-[65%]' : 'items-start max-w-[90%] md:max-w-[65%]'} ${m.is_new ? 'animate-msg-enter' : ''}`}>
              ...
              <div className={`${m.role === 'user' ? 'frosted-plum rounded-tr-sm bg-plum-high-contrast/90 text-white' : 'frosted-blush rounded-tl-sm bg-white/70'} px-6 py-4 rounded-2xl shadow-sm transition-all hover:shadow-md border border-white/20`}>
                <p className={`text-body-lg leading-relaxed ${m.role !== 'user' ? 'text-on-primary-fixed' : 'text-white'}`}>
                  {m.role !== 'user' ? (
                    <TypewriterText 
                      text={m.content} 
                      animate={!!m.is_new} 
