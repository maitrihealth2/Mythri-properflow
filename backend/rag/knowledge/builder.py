        ...
        for filename in files:
            ...
            if filename.endswith(".json"):
                with open(filepath, "r", encoding="utf-8") as f:
                    try:
                        ...
                        if isinstance(data, list):
                            ...
                            print(f"  ð {rel_path} â {len(data)} structured chunks")
                    except json.JSONDecodeError:
                        print(f"  â Error parsing {rel_path}")
                ...
                with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    print(f"  ð {rel_path} â {len(chunks)} text chunks")
                ...
                if content.strip():
                    ...
                    print(f"  ð {rel_path} â {len(chunks)} PDF chunks")
                ...
                if content.strip():
                    ...
                    print(f"  ð {rel_path} â {len(chunks)} DOCX chunks")
...
def ensure_knowledge_base_built(force: bool = False):
    ...
    if not force and is_knowledge_base_up_to_date():
        print("[RAG] â Knowledge Base is up to date. Skipping rebuild.")
    ...
    if os.path.exists(_INIT_LOCK_FILE):
        ...
        for _ in range(60):  # Wait up to 60 seconds
            ...
            if not os.path.exists(_INIT_LOCK_FILE):
                break
        print("[RAG] â Knowledge Base initialized by another worker.")
        _init_completed = True
        return None
    ...
    try:
        ...
        pass  # Non-fatal â proceed without lock

    print("[RAG] â Knowledge Base missing or outdated. Building now...")
    collection = None
    try:
        ...
        print(f"[RAG] â -  Knowledge Base build failed: {e}")
        print(f"[RAG] â -  Diagnostic Traceback:\n{error_trace}")
    finally:
        # Always release lock
        try:
    ...
    if collection is not None:
        try:
            ...
            print(f"[RAG] â Knowledge Base ready â {count} chunks indexed.")
        except Exception as e:
            print(f"[RAG] â -  Knowledge Base validation failed: {e}")
    else:
        print("[RAG] â -  Knowledge Base build returned no collection. Check logs above for errors.")
