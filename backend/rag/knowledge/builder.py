...
def extract_text_from_pdf(filepath: str) -> str:
    if not fitz:
        print("PyMuPDF (pymupdf) is not installed. Skipping PDF.")
        return ""
    text = ""
        ...
        for filename in files:
            ...
            if filename.endswith(".json"):
                with open(filepath, "r", encoding="utf-8") as f:
                    try:
                        ...
                        if isinstance(data, list):
                            ...
                            print(f"  [JSON] {rel_path} -> {len(data)} structured chunks")
                    except json.JSONDecodeError:
                        print(f"  [ERROR] Error parsing {rel_path}")
                ...
                with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
                    ...
                    print(f"  [TXT] {rel_path} -> {len(chunks)} text chunks")
                ...
                if content.strip():
                    ...
                    print(f"  [PDF] {rel_path} -> {len(chunks)} PDF chunks")
                ...
                if content.strip():
                    ...
                    print(f"  [DOCX] {rel_path} -> {len(chunks)} DOCX chunks")
...
def build_knowledge_base():
    ...
    try:
        ...
        print(f"[WARNING] Could not initialize EmbeddingFunction: {e}")
    ...
    for i in range(0, len(documents), batch_size):
        ...
        for attempt in range(5):
            try:
                ...
                print(f"[RAG] Error ({e}). Retrying in 5s (attempt {attempt+1}/5)...")
                import time
                time.sleep(5)
        else:
